          ...
          <div className="flex items-center">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-sm border-b-2 transition-colors ${
                isListActive ? "font-medium text-primary border-primary" : "text-muted-foreground hover:text-foreground border-transparent"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-sm border-b-2 transition-colors ${
                isTimelineActive ? "font-medium text-primary border-primary" : "text-muted-foreground hover:text-foreground border-transparent"
              }`}
            >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-56">
              <DropdownMenuRadioGroup value={selectedPE} onValueChange={setSelectedPE}>
                {PE_OPTIONS.map(opt => <DropdownMenuRadioItem key={opt} value={opt}>{opt}</DropdownMenuRadioItem>)}
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-44">
              <DropdownMenuRadioGroup value={selectedTribe} onValueChange={setSelectedTribe}>
                {TRIBE_OPTIONS.map(opt => <DropdownMenuRadioItem key={opt} value={opt}>{opt}</DropdownMenuRadioItem>)}
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-28">
              <DropdownMenuRadioGroup value={selectedYear} onValueChange={setSelectedYear}>
                {YEAR_OPTIONS.map(y => <DropdownMenuRadioItem key={y} value={y}>{y}</DropdownMenuRadioItem>)}
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          </DropdownMenu>
